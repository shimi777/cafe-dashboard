...
import pandas as pd
from html_to_excel import parse_html_transactions, create_daily_summary, create_detailed_transactions_df, \
...
with st.sidebar.expander("📝 עדכן יעדים", expanded=False):
    ...
    st.session_state.goals['actual_events'] = st.number_input(
        "כמות אירועים בפועל (עדכן ידנית)",
        value=st.session_state.goals['actual_events'],
        min_value=0,
        key="actual_events",
        help="עדכן את כמות האירועים שהתרחשו בחודש"
    )
        ...
        if len(transactions) == 0:
            ...
            with tab4:
                ...
                for week_num, week_start in enumerate(weeks_list, 1):
                    ...
                    revenue_percentage = (
                                week_revenue / total_monthly_revenue * 100) if total_monthly_revenue > 0 else 0
                ...
                st.info(
                    f"הגרף מציג את תרומת כל שבוע ליעד ההכנסה החודשי (יעד: ₪{st.session_state.goals['revenue_monthly']:,.0f}). הקו המפריד מציין את גבול היעד (100%)")
                ...
                display_df = weekly_contribution_df[
                    ['שבוע', 'הכנסה (₪)', 'עסקאות', 'ימים', 'משקל בחודש (%)', 'תרומה ליעד חודשי (%)']].copy()
                ...
                with stats_col2:
                    std_dev = (sum((x - avg_revenue) ** 2 for x in week_revenues) / len(
                        week_revenues)) ** 0.5 if week_revenues else 0
                    st.metric(
                        "סטיית תקן",
                        f"₪ {std_dev:,.0f}",
            ...
            with tab6:
                ...
                with goal_tab1:
                    ...
                    revenue_percentage = (
                                monthly_total_revenue / monthly_revenue_goal * 100) if monthly_revenue_goal > 0 else 0
                    ...
                    with col_events1:
                        actual_events = st.session_state.goals['actual_events']
                        events_goal = st.session_state.goals['events_monthly']
                        events_percentage = (actual_events / events_goal * 100) if events_goal > 0 else 0
                        st.metric(
                            "אירועים חודשיים",
                            f"{actual_events:,}",
                            delta=f"יעד: {events_goal} ({events_percentage:.0f}%)"
                        )

                    with col_events2:
                        st.metric(
                            "התקדמות אירועים",
                            f"{events_percentage:.1f}%",
                            delta="✅ הושג!" if events_percentage >= 100 else "⏳ קרוב ליעד" if events_percentage >= 80 else "❌ רחוק"
                        )
                    ...
                    fig_monthly.add_hline(y=100, line_dash="dash", line_color="red", annotation_text="יעד (100%)",
                                          annotation_position="right")
                    ...
                    progress_display = progress_display.rename(
                        columns={'קטגוריה': 'קטגוריה', 'יעד': 'יעד', 'בפועל': 'בפועל', 'אחוז התקדמות': 'התקדמות (%)'})
                ...
                with goal_tab2:
                    ...
                    if len(available_weeks) > 0:
                        ...
                        filtered_transactions = [t for t in transactions if pd.to_datetime(
                            t['date']).date() >= selected_week.date() and pd.to_datetime(t['date']).date() < (
                                                             selected_week + pd.Timedelta(days=7)).date()]
                        ...
                        weekly_revenue_percentage = (
                                    weekly_total_revenue / weekly_revenue_goal * 100) if weekly_revenue_goal > 0 else 0
                        ...
                        weekly_summary_data = {
                            ...
                            'אחוז התקדמות (%)': [item['אחוז התקדמות'] for item in weekly_progress] + [
                                weekly_revenue_percentage]
                        }
                        ...
                        fig_goals.add_hline(y=100, line_dash="dash", line_color="red", annotation_text="יעד (100%)",
                                            annotation_position="right")
                        ...
                        progress_display_weekly['אחוז התקדמות'] = progress_display_weekly['אחוז התקדמות'].apply(
                            lambda x: f"{x:.1f}%")
                        progress_display_weekly = progress_display_weekly.rename(
                            columns={'קטגוריה': 'קטגוריה', 'יעד': 'יעד', 'בפועל': 'בפועל',
                                     'אחוז התקדמות': 'התקדמות (%)'})
