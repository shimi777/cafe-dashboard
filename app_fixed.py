...
if uploaded_files and len(uploaded_files) > 0:
    ...
    try:
        ...
        if len(transactions) == 0:
            ...
            # Add WeekStart column for weekly analysis
            trans_df['Date'] = pd.to_datetime(trans_df['Date'])
            trans_df['WeekStart'] = trans_df['Date'] - pd.Timedelta(days=trans_df['Date'].dt.weekday)
            ...
            with tab6:
                ...
                with goal_tab2:
                    ...
                    if len(available_weeks) > 0:
                        ...
                        filtered_transactions = [t for t in transactions if datetime.strptime(t['date'], '%d/%m/%Y %H:%M').date() >= selected_week.date() and datetime.strptime(t['date'], '%d/%m/%Y %H:%M').date() < (selected_week + pd.Timedelta(days=7)).date()]
